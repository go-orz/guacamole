...
var handlers = map[string]handlerFunc{
	...
	"mouse": func(c *Client, args []string) error {
		x := parseInt(args[0])
		y := parseInt(args[1])
		c.display.moveCursor(x, y)
		return nil
	},

	"move": func(c *Client, args []string) error {
		layerIdx := parseInt(args[0])
		parentIdx := parseInt(args[1])
		x := parseInt(args[2])
		y := parseInt(args[3])
		z := parseInt(args[4])

		c.display.move(layerIdx, parentIdx, x, y, z)
		return nil
	},
	...
	"sync": func(c *Client, args []string) error {
		c.display.flush()
		if err := c.session.Send(guacamole.NewInstruction("sync", args...)); err != nil {
